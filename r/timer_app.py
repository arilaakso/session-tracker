...
from config import FONT_SIZE, LISTBOX_HEIGHT, SUMMARY_HEIGHT, WINDOW_WIDTH, WINDOW_HEIGHT, IDLE_THRESHOLD

print(f"FONT_SIZE: {FONT_SIZE}")
print(f"LISTBOX_HEIGHT: {LISTBOX_HEIGHT}")
print(f"SUMMARY_HEIGHT: {SUMMARY_HEIGHT}")
...
class SessionTracker:
    ...
    # 1. get_idle_duration() in utils.py uses Windows API to get time since last user input.
    # 2. check_idle() is called periodically (every CHECK_INTERVAL milliseconds).
    # 3. It compares idle time against IDLE_THRESHOLD (from config.py).
    # 4. If a session is running and idle time exceeds threshold, the session ends automatically.
    # 5. If no session is running and allow_auto_start is True, a new session starts when user becomes active.
    # 6. check_idle() is called recursively to ensure continuous monitoring.
